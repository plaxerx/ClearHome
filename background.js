...
let pendingSidePanelView = null;

function sidePanelIsOpen() { return sidePanelPort !== null; }

function requestSidePanelView(view) {
  if (sidePanelPort) {
    try { sidePanelPort.postMessage({ type: 'SHOW_VIEW', view }); return; } catch (e) {}
  }
  pendingSidePanelView = view;   
}

async function openSidePanel(tabId) {
  try {
    await chrome.sidePanel.open(tabId != null ? { tabId } : {});
    return true;
  } catch (e) { chDebug('sidePanel.open failed', e); return false; }
}
...
async function closeSidePanel(tabId) {
  ...
  } catch (e) { chDebug('sidePanel close failed', e); }
}

chrome.runtime.onConnect.addListener((port) => {
  if (port.name !== 'ch_sidepanel') return;
  sidePanelPort = port;
  if (pendingSidePanelView) {
    try { port.postMessage({ type: 'SHOW_VIEW', view: pendingSidePanelView }); } catch (e) {}
    pendingSidePanelView = null;
  }
...
chrome.action.onClicked.addListener(async (tab) => {
  if (sidePanelIsOpen()) await closeSidePanel(tab?.id);
  else                   await openSidePanel(tab?.id);
});

const FED_FUNDS_SPREAD = 2.75;
const FED_FUNDS_FALLBACK = 3.625; 
let _fedFundsCache = null;  
...
async function getMortgageRate() {
  ...
  try {
    ...
    if (stored.ch_mortgage_rate?.rate > 0) {
      ...
      if (stale) refreshMortgageRateInBackground(); 
      return _mortgageRateCache.rate;
    }
  } catch (e) {}
  refreshMortgageRateInBackground();
  try {
    const ff = await getFedFundsRate(); 
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'OPEN_SETTINGS') {
    ...
    requestSidePanelView('settings');
    return;
  }

  if (msg.type === 'OPEN_SIDE_PANEL') {
    openSidePanel(sender.tab?.id);
    requestSidePanelView(msg.view || 'analysis');
    return;
  }

  if (msg.type === 'CH_TO_PANEL') {
  ...
  if (msg.type === 'FINALIZE_RESULT') {
    ...
    try {
      ...
      if (mode === 'sold') {
        ...
        result.taxEstimate.rateUsed            = m.taxRateUsed;       
        result.taxEstimate.exemptionTotal      = m.totalExemption;    
...
async function fetchLandlordIntel(listingData) {
  ...
  if (listingData.daysOnMarket === 0)            result.scamRiskScore += 1; 
...
function computeOfferPrice(o) {
  ...
  const apprClamped = (rawAppr != null) ? Math.max(-50, Math.min(rawAppr, 200)) : null; 
  const priorSaleSane = o.originalSalePrice > 5000 && o.originalSalePrice < 100000000; 
  ...
  if (soldPpsfs.length >= 2) {
    ...
    compMedianPpsf = Math.round(median(listedPpsfs) * 0.96); 
  ...
  let sellerCreditPct = 1.0;                          
  if      (baseAggressiveness >= 15) sellerCreditPct = 3.0;  
  else if (baseAggressiveness >= 12) sellerCreditPct = 2.5;  
  else if (baseAggressiveness >= 8)  sellerCreditPct = 2.0;  
  else if (baseAggressiveness >= 5)  sellerCreditPct = 1.5;  
  if (regionalModifier < 0) sellerCreditPct = Math.max(1.0, sellerCreditPct - 0.5); 
  sellerCreditPct = Math.min(3.0, Math.max(1.0, sellerCreditPct));
  const sellerCredit = price > 0 ? Math.round((price * sellerCreditPct / 100) / 500) * 500 : 0; 
...
const ZIP3_TO_MSA = {
  ...
  '328':'ORL','347':'ORL','348':'ORL',  
  '326':'JAX','327':'JAX','322':'JAX','329':'JAX',               

  '330':'MIA','331':'MIA','332':'MIA','333':'MIA','334':'MIA','336':'TAM',
  '337':'TAM','338':'TAM','339':'TAM','341':'TAM',               
  '342':'TAM','346':'TAM',
  };
...
Break-Even Rent (1.0x): $${invBreakEven.toLocaleString()}/mo | Healthy Return Rent (1.25x): $${invViable.toLocaleString()}/mo`;
  ...
  const piPayment   = loanAmount > 0
    ...
    : 0;
  const monthlyTax  = Math.round(taxAfterReset / 12); 
  ...
  const affordability = {
    ...
    dtiWarning:           dtiHousing !== null ? dtiHousing > 28 : null, 
    mortgageRatePct:      effectiveRatePct,   
...
IMPORTANT: When citing PITI in keyHighlights or commentary, recalculate at your RECOMMENDED OFFER PRICE (suggestedOffer), not the list price. The UI displays affordability at the Clear Home offer. Example: if list is $565K but you recommend $548K, cite PITI at $548K.`;
  ...
  const premiumAdjustedPpsq = (() => {
    ...
    if (premiumSignals.some(p => p.feature === 'Pool')) {
      ...
      const poolAdj = warmStates.includes(propertyState || '') ? 26000 : 5000;  
      adj += poolAdj;
    }
    if (premiumSignals.some(p => p.feature === 'Water View/Waterfront'))       adj += price * 0.15;  
    if (premiumSignals.some(p => p.feature === 'Solar Panels'))                adj += price * 0.04;  
    if (premiumSignals.some(p => p.feature === 'Luxury Appliances'))           adj += 12000;  
    if (premiumSignals.some(p => p.feature === 'Full Renovation'))             adj += price * 0.10;  
    if (premiumSignals.some(p => p.feature === 'Recent Major System Replacement')) adj += 28000;  
    if (premiumSignals.some(p => p.feature === 'Impact Windows/Hurricane Protection')) {
      ...
      adj += coastalStates.includes(propertyState || '') ? 18000 : 4000;  
    }
...
async function analyzeRent(listingData, apiKey, TODAY, TODAY_ISO) {
  ...
  const estUtils = sqftNum > 0 ? Math.round(sqftNum * 0.065) : 120; 
  const rentersIns = 18; 
  const trueMonthly = price + estUtils + rentersIns;

  const fedFunds   = await getFedFundsRate();
  const invRateAnn = fedFunds + FED_FUNDS_SPREAD; 
  ...
  if (purchaseBasis > 0) {
    ...
    const n    = 360;
    const loan = purchaseBasis * INV_LTV; 
    ...
    const taxM   = Math.round(purchaseBasis * 0.012 / 12); 
    const insM   = Math.round(purchaseBasis * 0.006 / 12); 
    const maintM = Math.round(purchaseBasis * 0.010 / 12); 
    const utilM  = landlordPaysUtils ? 200 : 0; 
...
function estimateInsurance({ state, propertyType, price, yearBuilt, construction, hasHoa, zip, roofType, userOverride }) {
  ...
  if (stateCode === 'FL') {
    const isCoastalFL = (zipNum >= 33004 && zipNum <= 33482) || 
                        (zipNum >= 34201 && zipNum <= 34299) || 
                        (zipNum >= 32080 && zipNum <= 32084) || 
                        (zipNum >= 32250 && zipNum <= 32259) || 
                        (zipNum >= 34420 && zipNum <= 34491) || 
                        (zipNum >= 32401 && zipNum <= 32417) || 
                        (zipNum >= 32501 && zipNum <= 32560);   
    if (!isCoastalFL) {
      annual *= 0.72;
    ...
    const coastalZipPrefixes = {
      TX:  [[77500,77599],[77600,77650],[78336,78340],[78373,78374]], 
      LA:  [[70301,70400],[70460,70470]],   
      SC:  [[29400,29500],[29576,29579]],   
      NC:  [[28400,28500],[27954,27966]],   
      VA:  [[23300,23350]],                 
      NJ:  [[08200,08260],[07700,07760]],   
      NY:  [[11900,11980],[11690,11698]],   
      MA:  [[02540,02560],[02630,02670]],   
      ME:  [[04600,04650]],                 
      WA:  [[98200,98280],[98300,98370]],   
      CA:  [[90200,90280],[93400,93450],[94500,94570]], 
      HI:  [[96700,96999]],                 
    };
